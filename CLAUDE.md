...
**Current status:** Phase 8 complete. Cross-feed is fully implemented. `src/lib/crossfeed.ts` provides pure utility functions (`buildCrossFeedMessages`, `findLastAssistant`, `getNextCrossFeedRound`) for cross-feed message construction. `useProviderChat.send()` accepts optional `SendOptions` with `isCrossFeed`/`crossFeedRound` metadata, threaded through persistence. `App.tsx` orchestrates cross-feed via `handleCrossFeed` (reads latest assistant messages, constructs cross-feed content, dispatches to all three providers via `Promise.allSettled`). `useLiveQuery` reactively tracks cross-feed availability (all three providers must have assistant responses). `InputBar` has a cross-feed button (disabled until content available). `MessageBubble` renders cross-feed messages with dashed border and "Cross-feed" label. Settings, tri-model streaming, and conversation management from prior phases remain fully functional.
